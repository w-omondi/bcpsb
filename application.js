...
'${now}');`;
  ...
  db.query(query, (err, result) => {
    if (err) throw err;
    console.log("Applicants details registered");
...
const getLimitedApplications = (req, res) => {
  let query = `SELECT x.*,
  GROUP_CONCAT('[ ',y.award,' ',y.institution,' ',y.specialization,' ]' SEPARATOR '\n') AS certifications,
...
GROUP BY x.applicant_id ORDER BY x.timestamp DESC;`;
  db.query(fineQuery, (err, result) => {
    if (err) throw err;
    console.log("Querying all applications");
    res.json(result);
  });
};
...
const updateOtherPersonalDetails = (req, res) => {
  console.log("updating other personal");
...
const insertIntoAcademicQualifications = (req, res) => {
  console.log("Updating academic qualifications");
  const {
    applicantId,
    dateFrom,
    dateTo,
...
const updateJobDetails = (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) throw err;
      console.log("Job application details saved");
      res.json(result);
    }
  );
};
